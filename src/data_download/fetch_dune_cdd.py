"""
scripts/fetch_dune_cdd.py
使用 Dune API 获取 hourly CDD 数据
完全仿照 fetch_dune_active_addresses.py 风格
每次运行时自动追加到同一个文件（不会覆盖）
"""
...
# ==================== 配置区 ====================
QUERY_ID = 6909460   # ←←← 这里填入你新建的 CDD Query 的实际 ID
...
# 输出路径（和 netflow / active_addresses 保持一致结构）
OUTPUT_DIR = os.path.join(BASE_DIR, "../../data/raw")
os.makedirs(OUTPUT_DIR, exist_ok=True)

FINAL_FILE = os.path.join(OUTPUT_DIR, "cdd_1h.csv")
...
print(f"正在执行 CDD Query ID: {QUERY_ID} ...")
